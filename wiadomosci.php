	<?php include 'includes/header.php'; ?>
    <!-- HEADER:end -->
    
    

    
    <!-- SLIDER:begin -->
	<?php include 'includes/slider.php'; ?>
    <!-- SLIDER:end -->
    
    
            <div class="container">
       	<div class="row">

        	<div class="threecol">
            	<ul id="sub-nav">
                    	<li><a href="#" title="Wróć">Wróć</a></li>
                        <li><a href="#" title="Odebrane" class="active">Odebrane</a></li>
                        <li><a href="#" title="Usunięte">Usunięte</a></li>
                </ul>
            </div>

            <div class="ninecol last">
            	
            <div class="row">
                
       
                
            <!-- messages:begin -->
            <div class="sixcol" style="margin-right:0px;" >
            	<div class="messages">
                <form id="search-form-mess" action="szukaj-wiadomosci.php" method="post">
            		<div id="search-mess">
						<input type="text" placeholder="Wyszukaj użytkownika"/><input type="submit" id="search-button-mess" value="">
                   </div>
                 </form>
				

                <div class="panel-message read">
					<div class="photo"><img src="img/koci-magazyn-image.png" alt="Tytuł wiadomości" /></div>
                    <div class="content">
                        <h3><a href="#" title="Users name" class="fleft">Amber</a></h3>&nbsp;&nbsp;&nbsp;<span class="mess-hour">(9 godzin temu)</span>
						 <div class="message"><a href="#" title="Treść">Najlepsze kocie zabawy pomocne w utrzymaniu odpowiedniej pomocne w utrzymaniu odpowiednie wagi...</a></div>
                    </div>
                    <br class="clear">
				</div>
                
                <div class="panel-message unread">
					<div class="photo"><img src="img/koci-magazyn-image.png" alt="Tytuł wiadomości" /></div>
                    <div class="content">
                        <h3><a href="#" title="Users name" class="fleft">Amber</a></h3>&nbsp;&nbsp;&nbsp;<span class="mess-hour">(9 godzin temu)</span>
						 <div class="message"><a href="#" title="Treść">Najlepsze kocie zabawy pomocne w utrzymaniu odpowiedniej pomocne w utrzymaniu odpowiednie wagi...</a></div>
                    </div>
                    <br class="clear">
				</div>
		     </div>
            </div>
            <!-- messages:end -->
            		

            
            
            
            <!-- chat:begin -->
            
            
        <div class="sixcol last" id="chat">
        <div class="delete-message"><a href="javascript:void();" id="delete-message"  title="Usuń rozmowę">&#10006; &nbsp; Usuń rozmowę </a></div>
            <div class="chat-box">
            
            	<!-- message content:begin -->
            	<div class="send-message-first-user">
             		<div class="avatar-message fleft">
                		<img src="img/koci-magazyn-image.png" alt="User name"/>
                	</div>
					<div class="chat-arrow-left"></div>
                	<div class="message-content fleft">
                    	<div>
       				 		<p><a href="#" title="User name"  style="font-weight:bold;">Amber</a>&nbsp;&nbsp;&nbsp;<span class="mess-hour">dzisiaj 12:45</span></p>
                        </div>
                    	 <div class="message-text" >
                		Najlepsze kocie zabawy pomocne w utrzymaniu odpowiedniej pomocne w utrzymaniu odpowiednie wagi..Najlepsze kocie zabawy pomocne w utrzymaniu odpowiedniej pomocne w utrzymaniu odpowiednie wagi..Najlepsze kocie zabawy pomocne w utrzymaniu odpowiedniej pomocne w utrzymaniu 					iednie wagi..Najlepsze kocie zabawy pomocne w utrzymaniu odpowiedniej pomocne w utrzymaniu odpowiednie wagi..
                    	</div> 
                	</div>
                    <div style="clear:both;"></div>
				</div>                
                <!-- message content:end -->
                
                <!-- message content:begin -->
            	<div class="send-message-second-user">
             		<div class="avatar-message fright">
                		<img src="img/koci-magazyn-image.png" alt="User name"/>
                	</div>
                    
					 <div class="chat-arrow-right"></div>
                     
                	<div class="message-content fright">
                    	<div class="fright">
       				 		<p><a href="#" title="User name"  style="font-weight:bold;">Amber</a>&nbsp;&nbsp;&nbsp;<span class="mess-hour">dzisiaj 12:45</span></p>
                        </div><div style="clear:both;"></div>
                    	 <div class="message-text" >
                		Najlepsze kocie zabawy pomocne w utrzymaniu odpowiedniej pomocne w utrzymaniu odpowiednie wagi..Najlepsze kocie zabawy pomocne w utrzymaniu odpowiedniej pomocne w utrzymaniu odpowiednie wagi..Najlepsze kocie zabawy pomocne w utrzymaniu odpowiedniej pomocne w utrzymaniu 					iednie wagi..Najlepsze kocie zabawy pomocne w utrzymaniu odpowiedniej pomocne w utrzymaniu odpowiednie wagi..
                    	</div> 
                	</div>
                   
                    <div style="clear:both;"></div>
				</div>                
                <!-- message content:end -->
                
                <!-- message content:begin -->
            	<div class="send-message-first-user">
             		<div class="avatar-message fleft">
                		<img src="img/koci-magazyn-image.png" alt="User name"/>
                	</div>
					<div class="chat-arrow-left"></div>
                	<div class="message-content fleft">
                    	<div>
       				 		<p><a href="#" title="User name"  style="font-weight:bold;">Amber</a>&nbsp;&nbsp;&nbsp;<span class="mess-hour">dzisiaj 12:45</span></p>
                        </div>
                    	 <div class="message-text" >
                		<img src="img/typing-text.gif" alt="Użytkownik pisze wiadomość"/>
                    	</div> 
                	</div>
                    <div style="clear:both;"></div>
				</div>                
                <!-- message content:end -->
                
			</div>
            
            

            <div class="write-message">
            	<textarea type="text" placeholder="Napisz wiadomość" ></textarea>
                <a class="button button-teal fright" href="#" title="Więcej informacji">Wyślij &nbsp; &nbsp; &#x279d;</a>
                <div style="clear:both"></div>
            </div>
  
    </div>
    
    
            <!-- chat:end -->
			
          
            	</div>
            
        	</div>
            
            
		</div>
    
    
            </div>
    
    
    
    
    
    
    
    
    
    
    
	<!-- FOOTER:begin -->
	<?php include 'includes/footer.php'; ?>
    <!-- FOOTER:end -->