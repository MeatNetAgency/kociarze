    <br /><br />
    <div class="container" id="footer">
    	<div class="footer">
        	<div class="row">
            	<div class="threecol">
                	<h4>NAWIGACJA</h4>
                    	<ul>
                        	 <li><a href="#" title="Strona główna" >strona główna</a></li>
                            <li><a href="#" title="Aktualności" >akutalności</a></li>
                            <li><a href="#" title="Koci magazyn" >koci magazyn</a></li>
                            <li><a href="#" title="Wyzwanie" >wyzwanie</a></li>
                            <li><a href="#" title="Forum" >forum</a></li>
                            <li><a href="#" title="Blogi" >blogi</a></li>
                            <li><a href="#" title="Klub" >klub</a></li>
                        </ul>
                </div>
                <div class="threecol">
                	<h4>KONKURSY</h4>
                    	<ul>
                        	 <li><a href="#" title="3 - tygodniowe wyzwanie" >3 - tygodniowe wyzwanie</a></li>
                            <li><a href="#" title="Regulamin" >regulamin</a></li>
                        </ul>
                </div>
                <div class="sixcol last">
                	<h4>O NAS</h4>
                    <p>W Purina rozumiemy jak wiele znaczy dla Ciebie Twój kot,<br class="desktop" />ponieważ sami jesteśmy właścicielami kotów.</p>
                    <br />
                    <a href="#" title="Purina One" class="footer-image"><img src="img/purina-logo-footer.png" alt="Purina: Twój ulubieniec -nasza pasja"/></a>
                    <a class="button button-teal" href="#" title="Więcej informacji">Więcej informacji &nbsp; &nbsp; &#x279d;</a>
                    <br class="clear">
                </div>
            </div>
            <br />
            <div class="row footer-bottom">
            	<div class="sixcol copy">© Copyright 2012-2015 kociarze.pl</div>
                <div class="sixcol last links"><a href="#" title="Kontakt" >Kontakt</a><span class="desktop">&nbsp;&nbsp;|&nbsp;&nbsp;</span><br class="mobile" /><a href="#" title="Polityka prywatności" >Polityka prywatności</a><span class="desktop">&nbsp;&nbsp;|&nbsp;&nbsp;</span><br class="mobile" /><a href="#" title="Regulamin serwisu" >Regulamin serwisu</a></div>
            </div>
        </div>
    </div>
    </body>
</html>